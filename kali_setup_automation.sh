...
git stash && git pull && git stash drop
./gradlew build
echo "To run this java decompiler use the following command, replaxing the version number with your version number.\njava -jar ~/scripts/jd-gui/build/libs/jd-gui-1.6.6.jar\nAlso make sure that the jar " > '~/scripts/jd-gui/How to use this java decompiler.txt'
